...
type Course struct {
	...
	// CourseLocation holds the value of the "course_location" field.
...
func (*Course) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case course.FieldCourseName, course.FieldCourseLocation:
...
func (c *Course) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case course.FieldCourseLocation:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field course_location", values[i])
			} else if value.Valid {
				c.CourseLocation = value.String
			}
		default:
...
func (c *Course) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("course_location=")
	builder.WriteString(c.CourseLocation)
	builder.WriteByte(')')
