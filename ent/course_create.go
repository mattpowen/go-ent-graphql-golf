...
func (cc *CourseCreate) SetCourseLocation(s string) *CourseCreate {
	cc.mutation.SetCourseLocation(s)
	return cc
}
...
func (cc *CourseCreate) check() error {
	...
	if _, ok := cc.mutation.CourseLocation(); !ok {
		return &ValidationError{Name: "course_location", err: errors.New(`ent: missing required field "Course.course_location"`)}
	}
	return nil
}
...
func (cc *CourseCreate) createSpec() (*Course, *sqlgraph.CreateSpec) {
	...
	if value, ok := cc.mutation.CourseLocation(); ok {
		_spec.SetField(course.FieldCourseLocation, field.TypeString, value)
		_node.CourseLocation = value
	}
	return _node, _spec
}
